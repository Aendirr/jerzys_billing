...
if(Config.useCommand == true) then
    RegisterCommand('yenifatura', function ()
...
RegisterNUICallback('error', function ()
    QBCore.Functions.Notify('Lütfen Faturalandırmaya tekrar bakın!', 'error', 5000)
end)
...
RegisterNetEvent('jerzys_billing:client:sendBilling', function (ammount, title, text, name, cId)
    ...
    message = string.format([[
        <div class="jerzys_billing_text">%s</div><div class="jerzys_billing_ammount"> Amount: %s $</div> <br>Alttaki butonları kullanarak kabul edebilir ve reddedebilirsiniz.
        ]],text,ammount),
    button = {
        enabled = true,
...
RegisterNetEvent('jerzys_billing:client:error', function(data)
    if data.error == "online" then
        QBCore.Functions.Notify('Oyuncu şu anda çevrimiçi değil!', 'error', 5000)
    end

    if data.error == "player" then
        QBCore.Functions.Notify('Sen kimsin?', 'error', 5000)
    end
end)
